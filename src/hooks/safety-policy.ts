...
export function createSafetyPolicyHook(options: SafetyPolicyOptions = {}): HookHandler {
  ...
  return (input) => {
    ...
    if (tool === "shell" || tool === "start_service") {
      ...
      if (reason) return { blocked: true, warn: `Safety policy blocked ${tool}: ${reason}` }
      if (tool === "shell") return {}
      const cwd = typeof params.cwd === "string" ? params.cwd : ""
      if (cwd && !allowOutsideProject && !isPathInside(projectRoot, cwd)) {
        return { blocked: true, warn: `Safety policy blocked service cwd outside project: ${cwd}` }
      }
    }
    ...
    if (isSecretLikePath(path)) {
      return { blocked: true, warn: `Safety policy blocked access to sensitive path: ${path}` }
    }
